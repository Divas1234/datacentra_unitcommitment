...
num_dccs <- 24
blue_palette <- colorRampPalette(c("#ADD8E6", "#000080")) # LightBlue to Navy
dcc_colors <- blue_palette(num_dccs)
names(dcc_colors) <- paste0("DCC", 1:num_dccs)

# Create the stacked bar plot
q <- ggplot(data_long, aes(x = time, y = workload, fill = DCC)) +
  geom_bar(stat = "identity", color = "white", linewidth = 0.2, width = 0.75) +
