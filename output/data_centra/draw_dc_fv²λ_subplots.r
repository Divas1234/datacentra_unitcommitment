...
p <- ggplot(data_long, aes(x = time, y = workload)) +
  ...
  labs(
    x = "Time (h)",
    y = expression(paste(("F · V"^{2}), " ", lambda, " (p.u.)"))
  ) +
...
ggsave(plot = p, width = 8, height = 10, dpi = 300, filename = "d:/GithubClonefiles/datacentra_unitcommitment/output/data_centra/stacked_dc_fv²λ_subplots.pdf")
