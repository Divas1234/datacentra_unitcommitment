...
function add_dcc_constraints!(scuc, DataCentras, config_param, NS::Int, NT::Int, ND2::Int, num_sos::Int, dc_p,
	...
	if enable_active_response_flag == 0
		...
		@constraint(scuc, [s = 1:NS, t = 1:NT, i = 1:ND2], dc_fv²λ_2_minus[(s - 1) * ND2 + i, t] == sum(dcc_fv2lambda_2_minus_discrete[z] * weight_fv²λ_minus[(s - 1) * ND2 + i, t, z] for z in 1:num_sos))
