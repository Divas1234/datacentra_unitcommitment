...
num_dccs <- 24
grey_palette <- colorRampPalette(c("#e0e0e0", "#150e0e")) # LightGrey to DarkGrey
...
q <- ggplot(data_long, aes(x = time, y = workload, fill = DCC)) +
  geom_bar(stat = "identity", color = "#f9fdfd", linewidth = 0.2, width = 0.75) +
...
ggsave(plot = q, width = 6, height = 4, dpi = 300, filename = "./stacked_dcc_p4_plot.pdf")
